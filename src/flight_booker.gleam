...
fn view(model: Model) -> Element(Msg) {
  ...
  html.div([], [
    ...
    html.div([attribute.id("booking"), attribute.attribute("popover", "")], [
      html.text(format_booking(model)),
    ]),
  ])
}
...
fn parse_date(value: String) -> Value {
  case string.split(value, ".") {
    [day, month, year] ->
      ...
      |> result.map(fn(ts) { timestamp.to_calendar(ts, calendar.utc_offset) })
...
fn format_booking(model: Model) -> String {
  let booking = case model {
    Model(OneWay, start, _) -> "one-way flight on " <> format_value(start)
